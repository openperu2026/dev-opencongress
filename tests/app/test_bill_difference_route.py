...
Covers the request-time behaviour added on the diff branch:
...
from backend.core.enums import LegPeriod, Legislature, Proponents
    ...
    with session_factory() as db:
        db.add(
            Bill(
                ...
                legislature=Legislature.LEGISLATURA_2025_2,
                presentation_date=datetime(2022, 1, 1),
                title="Test bill",
                summary="",
                observations="",
                complete_text="",
...
def test_malformed_json_falls_through_cleanly(client, session_factory):
    _seed_bill(session_factory)
    _seed_step(session_factory)
    # Manually plant a truncated JSON string in the column.
    with session_factory() as db:
        db.add(
            BillDifference(
                step_id=1,
...
def test_renderer_exception_falls_through_cleanly(client, session_factory, monkeypatch):
    ...
    # Inject a renderer that always blows up.
    import app.routes.bills as bills_module
