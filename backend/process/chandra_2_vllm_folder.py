...
def get_images(pdf_file: str):
    ...
    try:
        ...
        print(
            f"Error processing PDF: {e}. Please ensure "
            f"'{pdf_file}' exists and is a valid PDF, or "
            f"provide an image file."
        )
...
def run_ocr_vllm(input_images, raw_filename):
    """
...
def process_pdf(pdf_file):
    ...
    run_ocr_vllm(input_images, raw_filename)
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        list(executor.map(process_pdf, pdf_lst))
