...
- **Bill Difference Tracker**: this feature identifies how the text of a bill changes over the course of its lifetime. The raw extracted text from each document often contains unnecessary text, which requires pre-processing: the bill text is scanned for key start and stop phrases (i.e. *"EL CONGRESO DE LA REPÚBLICA; Ha dado la Ley siguiente:"* and *"COMUNIQUESE AL SEÑOR PRESIDENTE DE LA REPUBLICA PARA SU PROMULGACIÓN"*) by `backend/process/billtext.py`, and the resulting body slice is stored in the `billtext` table. The orchestrator then pairs each step with the most recent text-bearing predecessor (deterministically ordered by `step_date ASC, id ASC`, walking back through text-less steps) and runs the three-layer hybrid diff in `backend/process/diff.py`: Layer 1 parses section structure and aligns nodes, Layer 2 line-diffs each aligned node, Layer 3 word-diffs each hunk. The structured JSON payload is persisted in `bill_differences.difference_content`. The diff stage is wrapped in an isolated `try/except` so a diff failure does not roll back the upstream bill/steps/documents load. The frontend renders this payload to HTML at request time and caches via ETag + `Cache-Control`; see [`bill_difference_contract.md`](./bill_difference_contract.md).
